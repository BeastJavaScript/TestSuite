...
(function() {
  var Result, TestCase, TestComponent, TestResult,
  ...
  TestComponent = (function() {
    ...
    TestComponent.prototype._trueTest = function(a) {
      if (a !== true) {
        return new Result(false, "failed to assert that " + (this.stringObject(a)) + " is true");
      } else {
        return new Result(true, "" + (this.stringObject(a)) + " is equal to true");
      }
    };

    TestComponent.prototype._same = function(a, b) {
      if (a === b) {
        return new Result(true, "asserted that " + (this.stringObject(a)) + " is " + (this.stringObject(b)));
      } else {
        return new Result(false, "failed to assert that " + (this.stringObject(a)) + " is " + (this.stringObject(b)));
      }
    };

    TestComponent.prototype._equals = function(a, b) {
      if (a instanceof Array && b instanceof Array) {
        return this._arrayEquals(a, b);
      }
      return this._same(a, b);
    };

    TestComponent.prototype._arrayEquals = function(a, b) {
      var k, result, v;
      if (a.length === b.length) {
        for (k in a) {
          v = a[k];
          if (!((result = this._equals(v, b[k])) && result.passed)) {
            return new Result(false, "failed to assert [" + (this.stringObject(a)) + "] is equal to [" + (this.stringObject(b)) + "] because property[" + (this.stringObject(k)) + "] is not equal");
          }
        }
      }
      return new Result(true, "asserted that [" + (this.stringObject(a)) + "] is equal to [" + (this.stringObject(b)) + "]");
    };

    TestComponent.prototype._isFunction = function(f) {
      if (typeof f === "function") {
        return new Result(true, "assert that " + (this.stringObject(f)) + " is a function");
      } else {
        return new Result(false, "failed to assert that " + (this.stringObject(f)) + " is a function");
      }
    };
    ...
    TestComponent.prototype.assertTrue = function(value) {
      var result;
      result = this._trueTest(value);
      if (result.passed) {
        ...
        return TestCase.result.push(new TestResult(TestCase.currentClassName, TestCase.currentName, result.message, TestCase.failed, TestCase.testAssert++));
      }
    };
    ...
    TestComponent.prototype.assertNull = function(value) {
      if (value === null) {
        return TestCase.result.push(new TestResult(TestCase.currentClassName, TestCase.currentName, "passed", TestCase.passed, TestCase.testAssert++));
      } else {
    ...
    TestComponent.prototype.assertNotNull = function(value) {
      if (value !== null) {
        return TestCase.result.push(new TestResult(TestCase.currentClassName, TestCase.currentName, "passed", TestCase.passed, TestCase.testAssert++));
      } else {
    ...
    TestComponent.prototype.assertEquals = function(test, value) {
    ...
    TestComponent.prototype.assertArrayEquals = function(array1, array2) {
      var result;
      result = this._arrayEquals(array1, array2);
      if (!result.passed) {
        return TestCase.result.push(new TestResult(TestCase.currentClassName, TestCase.currentName, result.message, TestCase.failed, TestCase.testAssert++));
      } else {
        return TestCase.result.push(new TestResult(TestCase.currentClassName, TestCase.currentName, result.message, TestCase.passed, TestCase.testAssert++));
      }
    };
    ...
    TestComponent.prototype.assertFunction = function(func) {
      var result;
      result = this._isFunction(func);
      if (result.passed) {
        return TestCase.result.push(new TestResult(TestCase.currentClassName, TestCase.currentName, result.message, TestCase.passed, TestCase.testAssert++));
      } else {
        return TestCase.result.push(new TestResult(TestCase.currentClassName, TestCase.currentName, result.message, TestCase.failed, TestCase.testAssert++));
    ...
    TestComponent.prototype.assertObjectEquals = function(obj1, obj2) {
      var e, result;
      try {
    ...
    TestComponent.prototype.deepObjectCompare = function(test, value) {
      var cmp, key, result;
      result = {};
      cmp = [];
      for (key in test) {
    ...
    TestComponent.prototype.assertSimilar = function(test, value) {
    ...
    TestComponent.prototype.assertSame = function(test, value) {
      if (test === value) {
        return TestCase.result.push(new TestResult(TestCase.currentClassName, TestCase.currentName, "passed", TestCase.passed, TestCase.testAssert++));
      } else {
    ...
    TestComponent.prototype.stringObject = function(o) {
      var element, str, strpack, _i, _len;
      str = "";
      strpack = [];
      if (o instanceof Array) {
        if (o.length < 10) {
          for (_i = 0, _len = o.length; _i < _len; _i++) {
            element = o[_i];
            strpack.push("" + (this.stringObject(element)));
          }
          str = strpack.join(", ");
        } else {
